...
import at.ac.ait.archistar.DigestHelper;
...
public class CheckpointManager {
	...
	public CheckpointManager(int serverId, ServerServerCommunication servers, SecurityMonitor secMonitor, int f) {
	...
	private void addCheckpointMessageToLog(CheckpointMessage msg) {
		...
		for(CheckpointMessage old : currentMessages) {
			if (!old.compatibleWith(msg)) {
				secMonitor.invalidCheckpointMessage(msg);
			}
		}
