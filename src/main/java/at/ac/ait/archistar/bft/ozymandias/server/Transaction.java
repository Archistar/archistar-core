...
import at.ac.ait.archistar.bft.ozymandias.commands.client.ClientFragmentCommand;
import at.ac.ait.archistar.bft.ozymandias.commands.client.ReadCommand;
import at.ac.ait.archistar.bft.ozymandias.commands.client.TransactionResult;
import at.ac.ait.archistar.bft.ozymandias.commands.client.WriteCommand;
...
public class Transaction implements Comparable<Transaction> {
	...
	public Transaction(AbstractCommand cmd, OzymandiasServer server, int f) {
		
		/* default stuff, valid for all commands */
		this.f = f;
		this.replica = server.getReplicaId();
		this.handler = server.getExecutionHandler();
	...
	private byte[] execute() {
		...
		try {
			if(clientCmd instanceof WriteCommand) {
				byte[] data = ((WriteCommand) clientCmd).getData();
				assert(this.handler != null);
				binResult = this.handler.putBlob(this.fragmentid, data);
			} else if (clientCmd instanceof ReadCommand) {
				assert(this.handler != null);
				binResult = this.handler.getBlob(this.fragmentid);
			}
	...
	public void tryAdvanceToPreprepared(boolean primary, ServerServerCommunication servers) {
	...
	public boolean tryAdvanceToPrepared(int lastCommited, ServerServerCommunication servers) {
